...
**Review follow-ups (open — schedule as the next `/sst-dev-cycle` cycle):**
- [x] 13.4 [easy] [should-fix] `SKILL.md:519` — Mode A.3's schema-spec template has no `variant:` or `profile:` declaration block at the top, even though Mode A.1 (line 451) instructs the agent to record the chosen profile "in the schema spec alongside `variant:`, at the top of the spec before any section headings." Schema specs written by Mode A will not have `profile:` on disk; future Mode B/C passes cannot apply profile defaults without re-asking the user, defeating the feature's stated purpose ("The profile signals intent so the agent doesn't ask about link style, front-matter requirements, and license tracking on every ingest"). Proposed fix: insert a 2-line YAML block at the top of the A.3 template (before `# <Domain> Knowledge Base`) showing `variant: <minimal|middle|scripted>` and `profile: <personal|publishable>`, matching the declaration format shown in §The profile axis.
- [ ] 13.5 [easy] [should-fix] `SKILL.md:908` — Mode B.1's "Note:" list omits the `profile:` declaration, so agents reading schema specs that now include `profile:` (written by the fixed A.3 template) have no explicit instruction to apply §The profile axis defaults (link style, front-matter requirements, license tracking) during ingest passes. Mode C.1 inherits the same gap via "Same as B.1." Without this hook, the feature's stated purpose — "the profile signals intent so the agent doesn't ask about link style, front-matter requirements, and license tracking on every ingest" — is not reliably realized in Mode B/C operations. Proposed fix: append `profile: (if present, apply §The profile axis defaults for link style and front-matter requirements)` to the "Note:" items in Mode B.1 (line 908).
